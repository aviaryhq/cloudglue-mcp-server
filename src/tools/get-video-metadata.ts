        ...
        const metadata = {
          ...
          video_info: file.video_info ? {
            ...
            duration_formatted: file.video_info.duration_seconds ? 
              `${Math.floor(file.video_info.duration_seconds / 60)}:${(file.video_info.duration_seconds % 60).toFixed(0).padStart(2, '0')}` : null,
          ...
          computed: {
            file_age_days: file.created_at ? 
              Math.floor((Date.now() - new Date(file.created_at).getTime()) / (1000 * 60 * 60 * 24)) : null,
            processing_duration_seconds: (file.processing_started_at && file.processing_completed_at && 
              typeof file.processing_started_at === 'string' && typeof file.processing_completed_at === 'string') ?
              Math.floor((new Date(file.processing_completed_at).getTime() - new Date(file.processing_started_at).getTime()) / 1000) : null,
          }
        };
