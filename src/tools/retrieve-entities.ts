...
export const schema = {
  ...
  limit: z
    ...
    .max(10)
    .describe(
      "Maximum number of entity records to return per request (1-10) with pagination. Use smaller numbers for initial exploration, larger for comprehensive analysis.",
    )
    .default(5),
  offset: z
    .number()
    .min(0)
    .describe(
      "Number of entity records to skip for pagination (e.g., offset=20, limit=20 gets entities 21-40). Use to page through large collections.",
    )
    .default(0),
  created_after: z
    .string()
    .describe(
      "Only include entities extracted after this date (YYYY-MM-DD format, e.g., '2024-01-15'). Useful for analyzing recent additions to a collection.",
    )
    .optional(),
  created_before: z
    .string()
    .describe(
      "Only include entities extracted before this date (YYYY-MM-DD format, e.g., '2024-01-15'). Useful for analyzing historical extractions.",
    )
    .optional(),
};
...
export function registerRetrieveEntities(
  ...
  server.tool(
    ...
    async ({ collection_id, limit, offset, created_after, created_before }) => {
      ...
      const entities = await cgClient.collections.listEntities(collection_id, {
        limit: fetchLimit,
        offset: 0,
      });
      ...
      if (created_after || created_before) {
        filteredEntities = filteredEntities.filter((entity: any) => {
          ...
          const entityDate = new Date(
            entity.created_at || entity.added_at || 0,
          );
